...
export async function GET(req: NextRequest) {
  const kind = req.nextUrl.searchParams.get("kind") || "saved"; // saved | liked
  const jar = await cookies();
  const token = jar.get(SESSION_COOKIE)?.value;
  if (!token) return NextResponse.json({ error: "auth_required", items: [] }, { status: 401 });
  const user = await getSessionUser(token);
  if (!user) return NextResponse.json({ error: "auth_required", items: [] }, { status: 401 });
  ...
  try {
    ...
    return NextResponse.json({ items: (rows as RowDataPacket[]).map(mapRow) });
  } catch (e) {
